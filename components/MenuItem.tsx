    ...
    <Card className="overflow-hidden border-border">
      <CardContent className="p-4 pt-4">
        <View className="flex-row gap-4">
          {item?.image ? (
            <View className="relative overflow-hidden rounded-lg shrink-0 w-[120px] h-[120px]">
              <AppImage uri={item.image} style={{ width: "100%", height: "100%" }} />
            </View>
          ) : null}
              ...
              <View className="relative">
                ...
                {cartQuantity > 0 ? (
                  <Badge className="absolute -top-2 -right-2 h-5 w-5 items-center justify-center p-0 bg-green-500 border border-green-600">
                    <Text className="text-xs text-white font-bold">{cartQuantity}</Text>
                  </Badge>
                ) : null}
              </View>
            </View>
