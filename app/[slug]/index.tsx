// ✅ app/(whatever)/index.tsx (COMPLETO / CORREGIDO)
// - Arregla: setIsLoading(false) en finally
// - PaymentMethods: fix del key (tenías mp_public_key duplicado y nunca probabas public_key)
// - Guarda order_id en web+native (no explota en native)
// - (opcional) slug fallback comentado: si querés default "hilda"
...
function expandCartToOrderItems(cart: CartLine[], catalog: MenuItemType[]) {
  const map = new Map<string, { product_id: string; name: string; description: string; quantity: number }>();

  const findByRemoteId = (pid: string) => catalog.find((p: any) => String(p.remote_id ?? p.id) === pid);
...
export default function Index() {
  const params = useLocalSearchParams<{ utm_campaign?: string; payment?: string; slug?: string }>();
  ...
  const [featured, setFeatured] = useState<{ id: number; name: string; description: string; image_url: string }[]>([]);
  ...
  const categories = useMemo(() => {
    ...
    return apiCategories.filter((c) => c.enabled).map((c) => toCategoryKey(c.name));
  }, [apiCategories]);
  ...
  // UTM + retorno pago
  useEffect(() => {
    const utm = params.utm_campaign;
    if (utm) setDeliveryLocation(String(utm));

    const payment = params.payment;
    if (payment === "success") setCartItems([]);
  }, [params.payment, params.utm_campaign]);

  // payment methods (1 vez por slug)
  useEffect(() => {
    if (!slug) return;
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        const mp = pms.find((x: any) => String(x.type).toUpperCase() === "MP" && x.enabled);
      ...
      try {
        ...
        const mapped: MenuItemType[] = prods
          ...
          .map((p: any) => {
            ...
            const id = Number.isFinite(numericId) ? numericId : Math.abs(hashCode(String(p.id)));
            const catName = enabledCats.find((c) => c.id === p.category_id)?.name ?? "OTRAS";
            return {
              id,
              name: p.name ?? "",
        ...
        if (cancelled) return;
        setPromotions(Array.isArray(promos) ? promos.filter((p: any) => p.enabled) : []);

        const featuredProds = await getPublicFeaturedProducts(slug);
        if (cancelled) return;

        const featuredMapped = (featuredProds || []).slice(0, 4).map((p: any) => {
          const numericId = Number(p.id);
          const id = Number.isFinite(numericId) ? numericId : Math.abs(hashCode(String(p.id)));
          return { id, name: p.name ?? "", description: p.description ?? "", image_url: p.image_url ?? "" };
        });
  ...
  const addPromotionToCart = (promo: PublicPromotion) => {
    setCartItems((prev) => {
      ...
      if (existing && existing.kind === "promo") {
        return prev.map((x) => (x.id === key ? { ...x, quantity: x.quantity + 1 } : x));
      }
      return [
        ...prev,
        {
  ...
  const updateQuantity = (key: string, quantity: number) => {
    ...
    setCartItems((prev) => prev.map((it) => (it.id === key ? { ...it, quantity } : it)));
  };
  ...
  const total = useMemo(() => cartItems.reduce((sum, it) => sum + it.price * it.quantity, 0), [cartItems]);
  ...
  return (
    <View className="flex-1 bg-background text-foreground">
      <ScrollView ref={scrollRef} className="flex-1" contentContainerStyle={{ paddingBottom: 160 }}>
        {/* Header */}
        <View className="bg-card/80 border-b border-black/10 px-4 py-2 items-center justify-center relative">
          <AppImage uri={(venue as any)?.logo_url} style={{ height: 48, width: 160 }} />

          <View className="absolute left-4 top-2">
            <Button onPress={() => setInfoModal(true)} variant="menu" size="icon" className="relative">
              <MaterialIcons name="info-outline" size={19} className="text-foreground" />
            </Button>
          </View>

          <View className="absolute right-4 top-2">
            <Button variant="menu" size="icon" className="relative" onPress={() => setIsCartOpen(true)}>
              <Feather name="shopping-cart" size={18} className="text-foreground" />
              {totalItems > 0 && (
        ...
        <View className="bg-background/95 px-4 text-foreground py-3 border-b border-black/10">
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View className="flex-row gap-2 items-center">
              {(categoriesWithPromos.length ? categoriesWithPromos : ["CARGANDO..."]).map((cat) => {
                const active = selectedCategory === cat;
                return (
                  <Pressable
                    key={cat}
                    onPress={() => categoriesWithPromos.length && scrollToCategory(cat)}
                    className={["px-5 py-2", active ? "border-b-2 border-accent" : ""].join(" ")}
                  >
                    <Text className={active ? "font-semibold text-foreground" : "text-black/60 font-semibold"}>
                      {cat}
                    </Text>
                  </Pressable>
                );
        ...
        {venue && !venue.service_active && (
          <View className="px-4 py-10 items-center">
            ...
            <Text className="opacity-70 mt-2">Este comercio no está recibiendo pedidos.</Text>
          </View>
        )}
        ...
        <View className="px-4 py-8">
          {promotions.length > 0 && (
            ...
            >
              <Text className="text-2xl text-foreground font-bold mb-6">PROMOCIONES</Text>
          ...
          <View className="gap-12">
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  <Text className="text-2xl text-foreground font-bold mb-6">{cat}</Text>

                  <View className="gap-4">
                    {items.map((it) => {
                      const productKey = `product:${String((it as any).remote_id ?? it.id)}`;
                      const cartItem = cartItems.find((c) => c.kind === "product" && c.id === productKey);
                      return (
                        <MenuItem
                          key={it.id}
                          item={it}
    ...
    />
      ...
      {!isCartOpen && !isCheckoutOpen && (
        <CartNotification totalItems={totalItems} total={total} onOpenCart={() => setIsCartOpen(true)} />
      )}
